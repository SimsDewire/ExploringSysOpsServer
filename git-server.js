...
module.exports = function(app, logger) {
	...
	git_daemon.stdout.on('data', function(data) {
		logger.log(data); // Log something if the git server says something
	});
	git_daemon.stderr.on('data', function(data) {
		logger.log(data); // Log something if the git reserer has errors
	});

	// This request returns a list of all available repositories
	app.get('/repos', function(request, response) {
		const repos = fs.readdirSync(git_path);
		const host = request.conn
